...
export default function Alertas() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const i = await DB.getInadimplentes()
    setInad(i); setLoading(false)
  }, [])
  ...
  return (
    <div>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16,flexWrap:'wrap',gap:10}}>
        ...
        <button className="btn btn-ghost btn-sm" onClick={load}><i className="ti ti-refresh"/>Atualizar</button>
      </div>
      ...
      <div className="g2">
        <div className="card">
          ...
          {!inad.length ? (
            <Empty icon="check" msg="Nenhuma inadimplência! Todos em dia." />
          ) : inad.map(({paciente,fatura,diasAtraso}) => {
            const msg = `Olá, ${paciente.nome.split(' ')[0]}! 🌿\n\nPassando para lembrar sobre o pagamento de ${fmtMoeda(fatura.valor)} referente às suas sessões.\n\nPIX: *${CONFIG.financeiro.chavePix}*\n\nObrigada! 😊`
            return (
              <div key={fatura.id} className="fin-inad-item">
                <div style={{flex:1,cursor:'pointer'}} onClick={() => router.push(`/prontuario?id=${paciente.id}`)}>
                  ...
                  <div style={{fontSize:11,color:'var(--danger)',fontWeight:600}}>
                    {diasAtraso > 0 ? `${diasAtraso}d em atraso` : 'Vence hoje'}
                  </div>
                ...
                <div style={{textAlign:'right',flexShrink:0}}>
                  ...
                  {paciente.fone && (
                    <a href={`https://wa.me/${paciente.fone}?text=${encodeURIComponent(msg)}`}
                      target="_blank" className="btn btn-ghost btn-sm" style={{marginTop:6,fontSize:11}}>
        ...
        <div className="card">
          ...
          <div style={{display:'flex',flexDirection:'column',gap:10}}>
            {[
              ...
              ['Mais atrasado', inad.length ? `${inad[0].diasAtraso}d` : '—', 'var(--warn)'],
            ].map(([l,v,c]) => (
            ...
            <button className="btn btn-primary btn-full" onClick={() => router.push('/financeiro')}>
              <i className="ti ti-report-money"/>Ver financeiro completo
            </button>
          </div>
        </div>
      </div>
