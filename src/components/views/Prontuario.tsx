...
function ProntuarioInner() {
  ...
  const MODELOS: Record<string,string> = {
    ...
    cbt_full: 'Humor/angústia (0-10):\n\nPensamento automático:\n\nEvidências a favor:\nEvidências contra:\n\nPensamento alternativo:\n\nResultado emocional:',
  }

  // Selecionar paciente quando pacientes do store carregam
  useEffect(() => {
    if (!pronto || !pacientes.length) return
    const alvo = idParam ? pacientes.find(p => p.id === idParam) : pacientes[0]
    if (alvo) selectPac(alvo)
  }, [pronto, idParam])

  const selectPac = useCallback(async (p: Paciente) => {
    setSel(p); setAba('ev'); setLoadingAba(true)
    try {
  ...
  const salvarEv = async () => {
    if (!sel || !formEv.trim()) { toast('Digite o texto','danger'); return }
    setSaving(true)
    try {
  ...
  const pagarFatura = async (fId: string) => {
    ...
    const [f, p] = await Promise.all([DB.getFaturas(sel.id), DB.getPaciente(sel.id)])
    setFaturas(f)
    if (p) setSel(p)
    reload('financeiro')
  }
  ...
  return (
    <div>
      <div style={{display:'flex',gap:10,marginBottom:14,flexWrap:'wrap'}}>
        <input placeholder="🔍 Buscar paciente..." value={busca}
          onChange={e=>setBusca(e.target.value)} style={{flex:'1 1 160px',minWidth:0}}/>
      ...
      <div className="pron-layout">
        ...
        <div className="patient-list-wrap">
          {!pronto ? [0,1,2,3,4].map(i=><Skel key={i} h={64}/>) : filtrados.length === 0
            ? <Empty icon="users" msg="Nenhum paciente encontrado"/>
            : filtrados.map(p => {
                const loc = getLocal(p.local_id)
                return (
                  <div key={p.id} className={`patient-item${sel?.id===p.id?' sel':''}`}
                    onClick={()=>selectPac(p)}>
                    <div className="pi-av" style={{background:loc?.cor||'var(--teal)'}}>
        ...
        <div className="pron-detail">
          {!sel
            ? <div className="card"><Empty icon="clipboard-text" msg="Selecione um paciente"/></div>
            : <>
              <div className="card">
                <div className="pac-header">
                  <div className="pac-av-lg" style={{background:getLocal(sel.local_id)?.cor||'var(--teal)'}}>
                ...
                <div className="pac-info-grid">
                  {[
                    ...
                    ['Telefone', sel.fone||'—'],['E-mail', sel.email||'—'],
                ...
                {sel.obs && <div style={{marginTop:10,fontSize:12,color:'var(--text2)',background:'var(--teal-light)',padding:'8px 12px',borderRadius:8}}><i className="ti ti-note" style={{marginRight:6}}/>{sel.obs}</div>}
              </div>
              ...
              <div className="card" style={{flex:1}}>
                ...
                {loadingAba ? <Skel h={120}/> : <>
                    ...
                    {showFormEv && (
                      <div style={{background:'var(--warm)',border:'1px solid var(--border)',borderRadius:8,padding:14,marginBottom:14}}>
                        ...
                        <div style={{display:'flex',gap:8}}>
                          ...
                          <button className="btn btn-ghost" onClick={()=>setShowFormEv(false)}>Cancelar</button>
                        </div>
                      </div>
                    )}
                    {evs.length===0 ? <Empty icon="note" msg="Nenhuma evolução registrada."/>
                      : evs.map(ev=>(
                  ...
                  {/* Financeiro */}
                  {aba==='fin' && (faturas.length===0 ? <Empty icon="coin" msg="Nenhuma fatura."/> : <>
                  ...
                  {/* Cartões */}
                  {aba==='cart' && (cartoes.length===0 ? <Empty icon="cards" msg="Nenhum cartão."/>
                    : cartoes.map(c=>{
                  ...
                  {/* Documentos */}
                  {aba==='docs' && (docs.length===0 ? <Empty icon="folder" msg="Nenhum documento."/>
                    : docs.map(d=>(
      ...
      <Modal open={modalPac} onClose={()=>setModalPac(false)} title="Novo paciente" icon="user-plus" large>
        ...
        <div className="field-row">
          <div className="field"><label>Nascimento</label><input type="date" value={formPac.nascimento} onChange={e=>setFormPac(p=>({...p,nascimento:e.target.value}))}/></div>
