...
select * from (values ('Ana Silva','AS','1991-03-15'::date,'F','5511999990001','F41.1','Presencial','aquarela','{adulto}'::text[],180::numeric,3,18,360::numeric,'Ansiedade generalizada.'),('Carla Nunes','CN','1997-07-22'::date,'F','5511999990002','','Presencial','unimed','{adulto,mulher}'::text[],200::numeric,6,12,0::numeric,''),('Marcos Lima','ML','1984-11-08'::date,'M','5511999990003','F32.0','Online','unimed','{adulto}'::text[],160::numeric,10,28,0::numeric,''),('Paula Mendes','PM','1999-07-03'::date,'F','5511999990004','','Presencial','anhangabau','{adulto,mulher}'::text[],220::numeric,15,8,0::numeric,''),('Rafael Costa','RC','1992-05-18'::date,'M','5511999990005','F40.1','Online','anhangabau','{adulto}'::text[],180::numeric,20,15,0::numeric,''),('Thiago Braga','TB','1996-02-14'::date,'M','5511999990006','','Online','aquarela','{adulto}'::text[],150::numeric,2,3,150::numeric,''),('João Pereira','JP','1980-09-30'::date,'M','5511999990007','','Presencial','aquarela','{adulto}'::text[],150::numeric,15,6,150::numeric,''),('Sofia Lima','SL','2017-04-12'::date,'F','5511999990008','F84.0','Presencial','aquarela','{crianca,neurodiverge}'::text[],190::numeric,10,10,0::numeric,'TEA nível 1.'),('Pedro Mendes','PD','2018-08-20'::date,'M','5511999990009','F90.0','Presencial','unimed','{crianca,neurodiverge}'::text[],190::numeric,15,6,0::numeric,'TDAH combinado.'),('Lúcia Ferreira','LF','1990-11-05'::date,'F','5511999990010','','Online','anhangabau','{supervisao}'::text[],250::numeric,25,14,0::numeric,'Supervisão quinzenal.')) as v(nome,avatar,nascimento,sexo,fone,cid,modalidade,local_id,perfil,valor_sessao,venc_dia,sessoes_total,devedor_total,obs)
...
select p.id,current_date,'16:00','Terapia Individual','Online','anhangabau','aguardando',180,false from pacientes p where p.nome='Rafael Costa' and not exists(select 1 from agendamentos limit 1);
